...
class ReleaseBranchCreatorIntegrationTest : FunSpec({
    ...
    fun createCreator(): ReleaseBranchCreator {
        ...
        return ReleaseBranchCreator(releaseExecutor, tagScanner, logger)
    }

    test("creates branches for multiple projects and pushes them atomically") {
        // given
    ...
    test("bumps version based on existing tags") {
        // given: app already has v0.1.0 tag
    ...
    test("bumps version past existing branch when no tag exists yet") {
        // given: app has branch v0.1.x but no tags (release not yet completed)
        val executor = GitCommandExecutor(logger)
        val releaseExecutor = GitReleaseExecutor(repoListener.repo.localDir, executor, logger)
    ...
    test("bumps version past both existing branch and tag") {
        // given: app has v0.1.0 tag AND v0.2.x branch (from a second releaseChanged before first completed)
        repoListener.repo.pushTag("release/app/v0.1.0")
        val executor = GitCommandExecutor(logger)
        val releaseExecutor = GitReleaseExecutor(repoListener.repo.localDir, executor, logger)
        releaseExecutor.createBranchLocally("release/app/v0.2.x")
        releaseExecutor.pushBranch("release/app/v0.2.x")
        releaseExecutor.deleteLocalBranch("release/app/v0.2.x")
        ...
        // then
        result.createdBranches shouldContainExactlyInAnyOrder listOf("release/app/v0.3.x")
        repoListener.repo.remoteBranchExists("release/app/v0.3.x") shouldBe true
    }
