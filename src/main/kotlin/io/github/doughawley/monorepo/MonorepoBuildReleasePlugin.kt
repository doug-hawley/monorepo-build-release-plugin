...
class MonorepoBuildReleasePlugin : Plugin<Project> {
    ...
    override fun apply(project: Project) {
        ...
        project.tasks.register("prepareReleases").configure {
            group = TASK_GROUP
            ...
            doLast {
                ...
                if (currentBranch != ext.primaryBranch) {
                    throw GradleException(
                        "prepareReleases must run on '${ext.primaryBranch}', " +
     ...
     * - If `prepareReleases` is requested (CI release build) →
